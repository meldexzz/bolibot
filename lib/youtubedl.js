  ...
  download: async (link, format, type = 'video') => {
    ...
    if (!id) {
      return {
        status: false,
        code: 400,
        error: "Donde esta la ID del video? no puedo extraerlo."
      };
    }
    ...
    try {
      ...
      while (retries < maxRetries) {
        ...
        if (data.le) {
          return {
            status: false,
            code: 400,
            error: "La duración del video es demasiado larga. El máximo es de 3 horas, no puedes superar eso, ¿entendido? 👍🏻"
          };
        }
        ...
        if (data.i === "blacklisted") {
          ...
          return {
            status: false,
            code: 429,
            error: `Limite de descargas diarias (${limit}) alcanzados, intente de nuevo mas tarde.`
          };
        }
      ...
      return {
        status: false,
        code: 500,
        error: "Ya intenté hacer la solicitud varias veces y sigue sin funcionar, así que dejaré la solicitud para más tarde, ¡hasta luego! "
      };
