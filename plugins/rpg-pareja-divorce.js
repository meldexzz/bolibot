...
conn.reply(m.chat, `@${m.sender.split('@')[0]} (${global.db.data.users[m.sender].name}) se Divorcio de @${targetUser.split('@')[0]} (${global.db.data.users[targetUser].name}) ahora están separados 🫣`, m, { mentions: [m.sender, targetUser] });
};
