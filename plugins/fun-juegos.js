...
if (command == 'follar' || command == 'violar') {   
    if (!text) return m.reply(`*Ingrese el @ o el nombre de la persona que quieras saber si te puedes ${command.replace('follar', 'follar').replace('violar', 'violar')}*`);

    // Verificamos si se mencionó a alguien o si hay un mensaje citado
    let user = m.mentionedJid[0] ? m.mentionedJid[0] : (m.quoted ? m.quoted.sender : null);
    
    // Si no se menciona a nadie, devolvemos un mensaje de error
    if (!user) return m.reply(`*No se encontró a la persona mencionada o citada. Por favor, menciona a alguien para continuar.*`);
